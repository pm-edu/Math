...
export default function ToeflReviewPage({ params }: { params: Promise<{ attemptId: string }> }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!res.ok || !data?.ok) {
        setErrorMsg(data?.message ?? "Failed to load the review.");
        setLoading(false);
        return;
      }
      setItems(data.items);
  ...
  return (
    <div data-theme="en" className="min-h-screen bg-[var(--background)]">
      ...
      <main className="mx-auto max-w-3xl px-6 py-16">
        <h1 className="text-3xl font-medium text-[var(--foreground)]">Question Review</h1>
        <p className="mt-1 text-sm text-[var(--secondary)]">
          Your answers, the correct answers, and explanations for every question.
        </p>
        ...
        {sections.map((section) => {
          ...
          return (
            <div key={section} id={`section-${section}`} className="mt-10 scroll-mt-20">
              <h2 className="text-lg font-semibold text-[var(--foreground)]">{SECTION_LABEL[section as ToeflSection]}</h2>
