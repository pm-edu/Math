...
const TEXT_FILE_EXTENSIONS = [".md", ".markdown", ".txt"];
function isTextFile(f: File): boolean {
  if (f.type.startsWith("text/") || f.type === "text/markdown") return true;
...
export default function AdminExtractPage() {
  ...
  const [genSolutions, setGenSolutions] = useState<{ done: number; total: number } | null>(null);
  ...
  // 저장 포함으로 체크된 draft들의 풀이를 순차적으로 AI 초안 생성한다(기존 /api/generate-solution 재사용).
  // 이 화면 자체가 검수 단계이므로, 생성된 초안은 바로 편집 가능한 상태로 채워진다(검토·수정 후 저장).
  async function handleGenerateSolutions() {
    const targets = drafts.map((d, i) => ({ d, i })).filter(({ d }) => d.include && d.content_text.trim());
    if (targets.length === 0) { setError("풀이를 만들 문제가 없습니다."); return; }
    setError(null); setMessage(null);
    setGenSolutions({ done: 0, total: targets.length });
    ...
    for (const { d, i } of targets) {
      try {
        const res = await fetch("/api/generate-solution", {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ content_text: d.content_text, answer: d.answer, problem_format: d.problem_format }),
        });
        const data = await res.json();
        if (res.ok && data.ok) updateDraft(i, { solution_text: data.solution });
      } catch {
        // 개별 실패는 건너뛰고 계속 진행 — 실패한 문제는 solution_text가 빈 채로 남는다.
      }
      done++;
      setGenSolutions({ done, total: targets.length });
    }
  }
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-6 py-16">
        ...
        <h1 className="mt-4 text-3xl font-medium text-[var(--foreground)]">PDF·이미지·마크다운에서 문제 추출</h1>
        <p className="mt-2 text-[var(--secondary)]">
          문제지 파일을 올리면 AI가 문제를 하나씩 읽어옵니다. 확인·수정 후 저장하세요.
          <br />
          정답이 틀릴 수 있으니 저장 전 꼭 검토해 주세요. 읽어온 문제들의 풀이는 "AI 풀이 일괄 생성"으로 한 번에 만들 수 있습니다.
        </p>
        ...
        <div className="mt-8 space-y-4 rounded-2xl border border-[var(--border-c)] bg-white p-6">
          ...
          <div>
            <label className="text-sm text-[var(--foreground)]">문제지 파일 (이미지, PDF, 또는 마크다운 .md/.txt)</label>
            <input
              type="file"
            ...
            <p className="mt-1 text-xs text-[var(--secondary)]">
              ...
              <br />
              직접 만든 .md/.txt 파일에 문제와 정답을 같이 적어두면, 이미지보다 훨씬 정확하게 정답까지 자동으로 채워집니다.
              <br />
              <b>.md/.txt에 수식을 쓸 때는 반드시 LaTeX로, $...$ 로 감싸주세요.</b> 예: 다음을 풀어라. $x^2 - 5x + 6 = 0$ (분수는 \frac&#123;1&#125;&#123;2&#125;, 제곱근은 \sqrt&#123;2&#125; 형태) — 이렇게 적어야 출판물처럼 또렷하게 렌더링됩니다.
            </p>
          </div>
            ...
            <div className="mt-10 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={!!genSolutions && genSolutions.done < genSolutions.total}
                ...
                >
                  {genSolutions && genSolutions.done < genSolutions.total
                    ? `풀이 생성 중... (${genSolutions.done}/${genSolutions.total})`
                    : "선택 문제 AI 풀이 일괄 생성"}
                </button>
                <button
                  onClick={handleSave}
            ...
            <ul className="mt-4 space-y-4">
              {drafts.map((d, i) => (
                ...
                >
                  ...
                  <label className="mt-3 block text-xs text-[var(--secondary)]">풀이 (선택 — AI 일괄생성 또는 직접 입력)</label>
                  <textarea
                    rows={2}
                    value={d.solution_text}
                    onChange={(e) => updateDraft(i, { solution_text: e.target.value })}
                    placeholder="AI 풀이 일괄 생성 버튼을 누르거나 직접 입력하세요."
                    className={inputClass}
                  />
                  {d.solution_text.trim() && (
                    <div className="mt-2 rounded-lg border border-dashed border-[var(--border-c)] bg-white p-2">
                      ...
                      <MathText text={d.solution_text} className="text-sm leading-relaxed text-[var(--foreground)]" />
                    </div>
                  )}
                </li>
              ))}
