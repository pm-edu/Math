...
- diagram 필드: 문제에 x 하나만의 구체적인 함수식이 나오면(예: "$f(x) = 9^x - 6 \cdot 3^{x} + 5$"처럼
  x 말고 다른 미지수가 없는 식) **풀이에 그래프가 꼭 필요하지 않아도** 그 함수의 그래프를 diagram에
  채우세요 — 학생이 문제를 이해하는 데 참고 자료가 되도록 하는 것입니다. 단, 문제 속 함수식에 k처럼
  x 말고 다른 미지수·상수가 남아있으면(값이 정해지지 않아 그래프를 그릴 수 없으므로) diagram을
  null로 둡니다. 특정 함수식이 아예 없는 문제(단순 부등식·방정식뿐이거나 함수가 불명확한 경우)도
  diagram은 null로 둡니다. 지금은 함수 그래프(function_graph)만 그릴 수 있으니, 삼각형·원 등 도형
  그림이 필요한 문제는 diagram을 null로 두고 그런 도형 문제는 아예 만들지 말고 다른 문제로 대신하세요.
