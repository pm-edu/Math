...
describe("generateCloze — 문맥 빈칸 채우기(Lv3) 문항 생성", () => {
  ...
  it("규칙활용형(accepted 등)도 찾아서 빈칸으로 바꾼다", () => {
    const item = generateCloze({ lemma: "accept", exampleEn: "She accepted the job offer." });
    expect(item?.prompt).toBe("She _____ the job offer.");
  });

  it("예문에서 단어를 못 찾으면(불규칙 활용 등) null을 반환한다", () => {
    const item = generateCloze({ lemma: "deny", exampleEn: "He denied breaking the window." });
