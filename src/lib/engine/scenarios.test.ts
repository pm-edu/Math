...
describe("시나리오 B — 벼락치기형: 하루에 몰아서 여러 번 풀고 다시 안 옴", () => {
  it("같은 세션에서 아무리 많이 맞혀도 레벨 1에서 멈춘다 (완전학습의 핵심 방어선)", () => {
    ...
    expect(mastery.lastSessionId).toBe(sessionId);
  });
...
describe("시나리오 C — 오래 쉰 복귀형: 한동안 안 하다가 돌아와서 잊어버림", () => {
  it("60일 쉬고 돌아와 오답을 내면, 레벨이 내려가고 FSRS 간격도 짧게 재조정된다", () => {
    ...
    const priorMastery: MasteryState = { level: 3, consecutiveWrong: 0, consecutiveCorrect: 0, lastSessionId: null };
  ...
  it("복귀 후 한 번 맞혔다고 바로 레벨이 돌아오지는 않는다 (다시 3세션 채워야 함)", () => {
    const afterWrong: MasteryState = { level: 2, consecutiveWrong: 1, consecutiveCorrect: 0, lastSessionId: null };
