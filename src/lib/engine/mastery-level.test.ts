...
describe("applyAnswer — 숙련도 5단계 사다리 승급/강등", () => {
  ...
  it("정확히: 레벨 1에서 서로 다른 3세션 연속 정답 시 레벨 2로 승급", () => {
    let state: MasteryState = { level: 1, consecutiveWrong: 0, consecutiveCorrect: 0, lastSessionId: null };
    let lastResult;
    for (const sessionId of ["s1", "s2", "s3"]) {
  ...
  it("같은 세션 안에서 여러 번 맞혀도 세션 1회로만 친다 (연속승급 방지)", () => {
    ...
    expect(state.lastSessionId).toBe("s1");
  });

  it("오답이면 레벨이 1 내려간다", () => {
    const state: MasteryState = { level: 3, consecutiveWrong: 0, consecutiveCorrect: 1, lastSessionId: "s1" };
  ...
  it("2연속 오답이면 레벨이 2 내려간다", () => {
    let state: MasteryState = { level: 4, consecutiveWrong: 0, consecutiveCorrect: 0, lastSessionId: null };
  ...
  it("최고 레벨은 5를 넘지 않는다", () => {
    let state: MasteryState = { level: 5, consecutiveWrong: 0, consecutiveCorrect: 0, lastSessionId: null };
    for (const sessionId of ["s1", "s2", "s3"]) {
  ...
  it("승급하면 연속 정답 세션 기록이 초기화된다 (다음 레벨에서 다시 3세션 필요)", () => {
    let state: MasteryState = { level: 1, consecutiveWrong: 0, consecutiveCorrect: 0, lastSessionId: null };
    for (const sessionId of ["s1", "s2", "s3"]) {
      state = applyAnswer(state, { sessionId, isCorrect: true }).next;
    }
  ...
  it("오답 한 번은 연속 정답 세션 기록을 초기화한다", () => {
    let state: MasteryState = { level: 2, consecutiveWrong: 0, consecutiveCorrect: 2, lastSessionId: "s2" };
  ...
  it("정답 후에는 연속 오답 카운터가 초기화된다", () => {
    let state: MasteryState = { level: 3, consecutiveWrong: 1, consecutiveCorrect: 0, lastSessionId: null };
    state = applyAnswer(state, { sessionId: "s1", isCorrect: true }).next;
