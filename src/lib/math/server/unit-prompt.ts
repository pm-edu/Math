...
- Triangle: {"kind":"triangle","vertices":[{"x":0,"y":0},{"x":6,"y":0},{"x":0,"y":8}],"labels":["A","B","C"],"sideLabels":{"ab":"6cm","ca":"8cm"},"rightAngleAt":0,"angleLabels":[{"at":1,"text":"50°"}],"extraPoints":[{"x":3,"y":0,"label":"D"}],"extraSegments":[{"from":{"x":0,"y":8},"to":{"x":3,"y":0},"dashed":true}]}
  (angleLabels marks a GIVEN angle measure at a vertex — index 0/1/2 into "vertices"/"labels". extraPoints/extraSegments are for a constructed point the problem mentions — an angle bisector's foot, a point on an extended side, etc. — draw the construction line with "dashed":true. Angle-chasing problems (isosceles triangles, angle bisectors, exterior angles, parallel lines) still need a triangle figure — use angleLabels/extraPoints/extraSegments instead of skipping the figure or describing them only in words.)
- Quadrilateral (parallelogram, rectangle, rhombus, trapezoid, kite, etc.) — a SEPARATE kind from "triangle". NEVER put 4 vertices into a "triangle" figure (it only accepts exactly 3 and will be rejected): {"kind":"quadrilateral","vertices":[{"x":0,"y":0},{"x":6,"y":0},{"x":8,"y":4},{"x":2,"y":4}],"labels":["A","B","C","D"],"sideLabels":{"ab":"6cm","cd":"6cm"},"angleLabels":[{"at":0,"text":"60°"}],"extraPoints":[{"x":4,"y":2,"label":"O"}],"extraSegments":[{"from":{"x":0,"y":0},"to":{"x":8,"y":4},"dashed":true},{"from":{"x":6,"y":0},"to":{"x":2,"y":4},"dashed":true}]}
  (vertices go in order around the shape, A→B→C→D→A. sideLabels keys are "ab"/"bc"/"cd"/"da". extraPoints/extraSegments work the same as triangle — use them for a diagonal or its intersection point.)
