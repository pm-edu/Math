...
export default function SessionPlayer({
  ...
  backHref: string;
  backLabel: string;
  // 교정학습: 직전과 다른 문항 유형을 강제로 지정하고 싶을 때(gate.buildCorrectiveQueue 결과)
  forcedItemTypeByWord?: Record<string, ItemType>;
  onFinished?: () => void;
  ...
  const [current, setCurrent] = useState<CurrentItem | null>(() =>
    queue.length > 0 ? buildItem(queue[0], queue, forcedItemTypeByWord?.[queue[0].content.id]) : null
  );
  ...
  function next() {
    ...
    if (nextPos < queue.length) {
      setCurrent(buildItem(queue[nextPos], queue, forcedItemTypeByWord?.[queue[nextPos].content.id]));
    } else {
      setCurrent(null);
      finishSession();
      onFinished?.();
      ...
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        ...
        <div className="mt-8 flex justify-center gap-2">
          <Link
            ...
            className="inline-block rounded-full bg-[var(--pink)] px-6 py-3 text-sm font-medium text-[var(--pink-dark)]"
          >
            {backLabel}
          </Link>
          {extraFinishedAction && (
            <Link
              href={extraFinishedAction.href}
