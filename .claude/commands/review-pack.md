...
그다음 아래 세 가지를 **항상 추가로** 찾아 목록에 더한다(2026-08-27 교차검증 두 라운드에서
리뷰어가 "이게 없어서 판단을 못 했다"고 지적한 것들 — 표에 없어도 매번 확인):
...
- **호출하는 관리자 화면 컴포넌트** (2026-08-27 재검증 D2) — 위에서 모은 API 라우트를 `fetch()`로
  부르거나, 위에서 모은 `src/lib/**` 함수를 직접 import하는 `src/app/admin/**/*.tsx`·
  `src/components/**/admin/**/*.tsx`를 `grep -rl`로 찾아 포함한다. 관리자 화면이 스펙과 다른
  방식으로 데이터를 넘기고 있는지는 화면 코드를 봐야 판단할 수 있다(예: 이번 라운드의 regrade
  라우트 지적처럼, "화면이 어떤 클라이언트로 호출하는지"가 보안 판단에 직결됨).
...
**스펙 섹션 자동 추출(2026-08-27 재검증 D1)**: 1단계에서 모은 모든 파일의 주석에서 `§\d+`
패턴(예: `§6`, `§12`)을 정규식으로 전부 뽑는다. 표에 적힌 섹션(그리고 §6·§8)에 없는 새 번호가
나오면, 그 섹션도 `docs/toefl-spec.md`에서 찾아 2단계 스펙 발췌 목록에 추가한다 — 코드 주석이
"§9 참고"라고 적어놨는데 정작 팩에 §9가 없으면, 리뷰어가 그 근거를 확인할 방법이 없다.
...
## 4-1. 포함 파일 목록 자동 검사 (2026-08-27 재검증 D3)

저장 직전, "## 포함 파일" 목록의 각 경로가 본문에 정확히 `## FILE: <그 경로>` 헤더로 실제로
들어있는지 하나씩 확인한다(예: `grep -c "## FILE: <경로>"`가 1이어야 함 — 0이면 목록에는
적었는데 실제로 안 넣은 것, 2 이상이면 중복 삽입). 반대 방향도 확인한다 — 본문에 `## FILE:`
헤더가 있는데 목록에는 없는 경우도 찾는다. 둘 중 하나라도 어긋나면 저장하지 말고 먼저 맞춘다
(목록은 사람이 읽는 색인이자 리뷰어가 "빠진 게 없는지" 확인하는 최소 근거라, 실제 내용과
달라지면 팩 자체의 신뢰도가 깨진다).
